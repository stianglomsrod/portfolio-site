...
Most previously-blocked claims are now primary-source-backed. Remaining care items are about *wording and discretion*, not missing evidence:
...
GitHub Copilot Auto can safely implement using `DNB_IMPLEMENTATION_BRIEF_FOR_AUTO.md`, because the copy is pre-rated and the agent only assembles — it does not invent claims.
  4. Then proceed to implementation chunk: DNB page skeleton with agentic workflow and Klar sections only.
