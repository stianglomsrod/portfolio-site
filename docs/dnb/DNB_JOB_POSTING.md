# DNB Job Posting — Software Engineer, AI-First Engineering

> Source of record for role-fit. Stored verbatim as provided by the user on 2026-06-20.
> This is a public job advertisement, so it is tracked (not sensitive). Application deadline: 01.07.2026.
> Use this file as the canonical role-fit reference. It supersedes the earlier "exact posting not stored" proxy note.
...
### Application process notes (from posting)

- Only CV + short answers to position-related questions are required. A cover letter is optional (can be uploaded as an attachment).
- Candidates are reviewed on a rolling basis up to the deadline.
- DNB runs a background check (via Semac) with the applicant's consent to verify CV/documentation.
- DNB never asks for BankID information in the application process.

---

## Role-fit reading (how this maps to Stian's positioning)

This section is interpretation for agents — see `DNB_INSIGHT_SYNTHESIS.md` for the full strategy.
...
- Distributed systems, build/CI infrastructure, platform engineering at scale.
- GitHub Actions/API automation depth, LLM-API tool-calling, MCP / agentic frameworks (LangGraph, Strands).
- Open-source/innersource track record at organisational scale.
- Productivity metrics instrumentation (cycle time, review throughput).
