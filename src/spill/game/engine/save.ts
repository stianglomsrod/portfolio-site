...
/* Skjemaversjon: bumpes når innholds-/quest-id-er endres, slik at gamle
   saves forkastes rent i stedet for å laste inkonsistent progresjon. */
export const SAVE_VERSJON = 2;
...
export function loadState(pack: ContentPack): SaveState | null {
  ...
  try {
    ...
    if (parsed.versjon !== SAVE_VERSJON) return null;
    return parsed;
  } catch {
    return null;
  }
